...
namespace BlackCat\Database\Packages\PolicyAlgorithms\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PolicyAlgorithms\Dto\PolicyAlgorithmDto;
use BlackCat\Database\Packages\PolicyAlgorithms\Mapper\PolicyAlgorithmDtoMapper;
 ...
 */
final class PolicyAlgorithmsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PolicyAlgorithmRepository $policyAlgorithmRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'policy_algorithms' => $this->policyAlgorithmRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PolicyAlgorithms\ModuleException("Record {$idStr} not found for locking.");
            }
