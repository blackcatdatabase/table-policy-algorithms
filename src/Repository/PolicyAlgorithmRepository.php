...
namespace BlackCat\Database\Packages\PolicyAlgorithms\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PolicyAlgorithms\Definitions;
use BlackCat\Database\Packages\PolicyAlgorithms\Criteria;
use BlackCat\Database\Packages\PolicyAlgorithms\Dto\PolicyAlgorithmDto as Dto;
use BlackCat\Database\Packages\PolicyAlgorithms\Mapper\PolicyAlgorithmDtoMapper as RowMapper;
...
final class PolicyAlgorithmRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PolicyAlgorithms\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'policy_id', 'algo_id', 'role' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PolicyAlgorithms\Definitions::table());
            return $bulk->upsertMany($rows, [ 'policy_id', 'algo_id', 'role' ], [ 'weight', 'priority' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PolicyAlgorithms\Dto\PolicyAlgorithmDto|null */
    public function getByPolicyIdAndAlgoIdAndRole(int $policyId, int $algoId, string $role, bool $asDto = false): array|\BlackCat\Database\Packages\PolicyAlgorithms\Dto\PolicyAlgorithmDto|null {
        $row = $this->getByUnique([ 'policy_id' => $policyId, 'algo_id' => $algoId, 'role' => $role ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PolicyAlgorithms\Mapper\PolicyAlgorithmDtoMapper::fromRow($row);
    }
    public function existsByPolicyIdAndAlgoIdAndRole(int $policyId, int $algoId, string $role): bool {
        $where = 't.' . Ident::q($this->db, 'policy_id') . ' = :uniq_policy_id' . ' AND ' . 't.' . Ident::q($this->db, 'algo_id') . ' = :uniq_algo_id' . ' AND ' . 't.' . Ident::q($this->db, 'role') . ' = :uniq_role';
        return $this->exists($where, [ 'uniq_policy_id' => $policyId, 'uniq_algo_id' => $algoId, 'uniq_role' => $role ]);
    }
    /** @return int|string|null */
    public function getIdByPolicyIdAndAlgoIdAndRole(int $policyId, int $algoId, string $role) {
        $row = $this->getByPolicyIdAndAlgoIdAndRole($policyId, $algoId, $role);
        return $row ? ($row['policy_id algo_id role'] ?? null) : null;
    }
