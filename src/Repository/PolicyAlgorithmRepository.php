...
use BlackCat\Database\Packages\PolicyAlgorithms\Repository\PolicyAlgorithmRepositoryInterface;
...
class PolicyAlgorithmRepository implements PolicyAlgorithmRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'policy_id', 'algo_id', 'role' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PolicyAlgorithms\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'weight', 'priority' ]);
          return count($rows);
        }
    ...
    public function getByPolicyIdAndAlgoIdAndRole(int $policyId, int $algoId, string $role, bool $asDto = false): array|\BlackCat\Database\Packages\PolicyAlgorithms\Dto\PolicyAlgorithmDto|null {
        return $this->getByUnique([ 'policy_id' => $policyId, 'algo_id' => $algoId, 'role' => $role ], $asDto);
    }
    ...
    public function getIdByPolicyIdAndAlgoIdAndRole(int $policyId, int $algoId, string $role) {
        $row = $this->getByPolicyIdAndAlgoIdAndRole($policyId, $algoId, $role, false);
        if (!is_array($row)) { return null; }
        return $row['policy_id algo_id role'] ?? null;
    }
