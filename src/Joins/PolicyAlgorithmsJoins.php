...
namespace BlackCat\Database\Packages\PolicyAlgorithms\Joins;
 ...
 */
final class PolicyAlgorithmsJoins {
    ...
    /**
     * FK: policy_algorithms -> encryption_policies
     * LEFT JOIN vw_encryption_policies AS $as ON $as.id = $alias.policy_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinEncryptionPolicies(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_encryption_policies AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.policy_id' . ' ', []];
    }
    /**
     * FK: policy_algorithms -> crypto_algorithms
     * LEFT JOIN vw_crypto_algorithms AS $as ON $as.id = $alias.algo_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinCryptoAlgorithms(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_crypto_algorithms AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.algo_id' . ' ', []];
    }
